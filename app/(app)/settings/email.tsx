...
export default function EmailSettingsScreen() {
  ...
  const canSend = emailIsValid && !isSending && !isVerifying;
  const canResend = Boolean(pendingEmail) && !isSending && !isVerifying;
  ...
  const handleSend = async () => {
    ...
    try {
      await sendCurrentUserEmailVerification({ email: normalizedEmail });
  ...
  const handleResend = async () => {
    ...
    try {
      await sendCurrentUserEmailVerification({ email: pendingEmail });
      setAttemptKey((current) => current + 1);
  ...
  const handleComplete = async (token: string) => {
    ...
    try {
      const updatedUser = await completeCurrentUserEmailVerification({
        email: pendingEmail,
        token,
      });
